...
class temp(object):   
    ...
    TEMP_INVITE_LINKS = {}
    REQ_LINKS = {}
    LAST_SEARCH = {}  # {user_id: raw search text} — used to build external "Download Now" link on file-limit reply
...
def normalize_season_episode(search_raw: str) -> str:
    """
    Normalize natural-language season/episode phrases to SxxExx format.
    Examples:
        "season 1"        -> "S01"
        "Season 12"       -> "S12"
        "s 2"             -> "S02"
        ...
        "ep 04"           -> "E04"
        "e 5"             -> "E05"
        "season 1 episode 2" -> "S01 E02"
    Leaves already-normalized tokens (S01, E02) untouched.
    """
    if not search_raw:
        return search_raw
    text = search_raw
    # season N / seasons N / s N (with optional space) -> S0N
    text = re.sub(
        r'\bseasons?\s*0*(\d{1,2})\b',
        lambda m: f"S{int(m.group(1)):02d}",
        text,
        flags=re.IGNORECASE,
    )
    # standalone "s 1" (letter s + space + digits) -> S01 (avoid touching S01 already)
    text = re.sub(
        r'\bs\s+0*(\d{1,2})\b',
        lambda m: f"S{int(m.group(1)):02d}",
        text,
    ...
    # episodes N / episode N / ep N / e N -> E0N
    text = re.sub(
        r'\bepisodes?\s*0*(\d{1,3})\b',
