...
from info import (
    ...
    COVERX, PROTECT_CONTENT, DELETE_TIME, IS_FILE_LIMIT, FILES_LIMIT, PREMIUM_STREAM_MODE, STREAM_MODE, SUPPORT_CHAT_ID, REQST_CHANNEL,
    LOG_API_CHANNEL, SHORTENER_API, SHORTENER_API2, SHORTENER_API3, SHORTENER_WEBSITE, SHORTENER_WEBSITE2, SHORTENER_WEBSITE3,
    EXTERNAL_FILE_BOT_USERNAME,
)
...
async def start(client, message):
    ...
    try:
        ...
        if len(message.command) == 2 and message.command[1] == "ads":
            try:
                ads_string, ads_name, impression = await mdb.get_advirtisment()
                ads_photo = await mdb.get_ads_link()
                buttons = [[InlineKeyboardButton("❌ ᴄʟᴏsᴇ ❌", callback_data="close_data")]]
                reply_markup = InlineKeyboardMarkup(buttons)
                if ads_string:
                    try:
                        fallback = ads_photo or (random.choice(PICS) if PICS else None)
                        await message.reply_photo(
                            photo=fallback,
                            caption=ads_string,
                            reply_markup=reply_markup,
                ...
                logger.exception("start=ads failed: %s", _e)
            return
        ...
        if not await db.has_premium_access(user_id):
            try:
                ...
                if settings.get("is_verify", IS_VERIFY) and (not user_verified or is_second_shortener or is_third_shortener):
                    ...
                    verify_text = msg.format(message.from_user.mention)
                    try:
                        ...
                        logger.exception("ads inject (verify) failed: %s", _e)
        ...
        try:
            if (
                ...
                if current_file_count >= FILES_LIMIT:
                    ...
                    ext_bot = (EXTERNAL_FILE_BOT_USERNAME or "").lstrip("@")
                    limit_buttons = []
                    if slug and ext_bot:
                        limit_buttons.append([
                            InlineKeyboardButton(
                                "📥 ᴅᴏᴡɴʟᴏᴀᴅ ɴᴏᴡ",
                                url=f"https://t.me/{ext_bot}?start=getfile-{slug}",
                            )
                        ])
                    limit_buttons.append([
                    ...
                    try:
                        ...
                        logger.exception("ads inject (limit) failed: %s", _e)
                    await m.reply_text(
                        limit_text,
                        parse_mode=enums.ParseMode.HTML,
...
# =====================================================================
# File Limit Admin Commands (ported from SiliconBotz)
# =====================================================================

@Client.on_message(filters.command("resetlimit") & filters.user(ADMINS))
async def _fl_reset_all_limits(client, message):
    try:
        ...
        await message.reply_text(
            f"<b>❌ Error resetting limits: {e}</b>",
            parse_mode=enums.ParseMode.HTML,
        )


@Client.on_message(filters.command("resetuser") & filters.user(ADMINS))
async def _fl_reset_user_limit(client, message):
    if len(message.command) < 2:
        return await message.reply_text(
    ...
    try:
        ...
        old_limit = filelimitdb.get_file_limit(user_id)
        filelimitdb.reset_file_limit(user_id)
        await message.reply_text(
            (f"<b>✅ ʀᴇsᴇᴛ ꜰɪʟᴇ ʟɪᴍɪᴛ ꜰᴏʀ ᴜsᴇʀ <code>{user_id}</code>\n\n"
             f"ᴘʀᴇᴠɪᴏᴜs: {old_limit}\nᴄᴜʀʀᴇɴᴛ: 0</b>"),
            parse_mode=enums.ParseMode.HTML,
        )
    except ValueError:
        await message.reply_text(
...
async def _fl_check_user_limit(client, message):
    if len(message.command) < 2:
        return await message.reply_text(
    ...
    try:
        user_id = int(message.command[1])
        current = filelimitdb.get_file_limit(user_id)
        await message.reply_text(
            (f"<b>📊 ꜰɪʟᴇ ʟɪᴍɪᴛ sᴛᴀᴛᴜs ꜰᴏʀ <code>{user_id}</code>\n\n"
             f"ᴄᴜʀʀᴇɴᴛ: {current}/{FILES_LIMIT}\n"
             f"ʀᴇᴍᴀɪɴɪɴɢ: {max(0, FILES_LIMIT - current)}</b>"),
            parse_mode=enums.ParseMode.HTML,
        )
    except ValueError:
        await message.reply_text(
